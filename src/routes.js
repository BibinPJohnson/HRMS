...
router.post('/getEmployeeSalary', async (req, res) => {
  try {
    const result = await getEmployeeSalary(req.body); // Assuming the necessary data is in the body
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.post('/getEmployeeSalaryDetails', async (req, res) => {
  try {
    const result = await getEmployeeSalaryDetails(req.body); // Assuming the necessary data is in the body
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// const express = require('express');
// const getEmployeeReport = require('./resolvers/getEmployeeReport');
// const getEmployeeSalary = require('./resolvers/getEmployeeSalary');
// const getEmployeeSalaryDetails = require('./resolvers/getEmployeeSalaryDetails');
